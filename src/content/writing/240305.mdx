...
Fields for my posts are established by a data model defined in a config file in a content directory in the source files of my website. This config file describes a javascript object that comprises of fields of a variety of types, which correspond to the fields I can use for my blog posts.
...
(Author's note: While writing this in a dark mode context, I realized the links looked terrible and spend 20 minutes fiddling with the styling to get it feeling better. There's still a bit of mediation I need to do between how my front matter colors are applied to things like links and code blocks and the like, which tailwind has taken over via its typography plugin. It might be time to get rid of it since I don't really use it "correctly".)
...
I don't think it's neccesarily a perfect magnet for capital investment yet, but I am interested in obctaining moderate funding to spend more time cultivating the idea, establishing stronger technical roots, making it realer, making it valuable, etc.
